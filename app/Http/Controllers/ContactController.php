...
class ContactController extends Controller
{
    public function index()
    {
        try {
            ...
            $homePage = Page::where('slug', 'contact')->firstOrFail();
            ...
            Log::error('Database error in ContactController: ' . $e->getMessage());
            $sections = collect();
            $dbError = 'Koneksi ke database gagal.';
        } 
        catch (\Exception $e) {
            Log::error('Error umum di ContactController: ' . $e->getMessage());
            $sections = collect();
            $dbError = 'Terjadi kesalahan tak terduga.';
        }

        // 4. Kirim variabel ke view
        return view('pages.contact', compact('sections', 'dbError'));
    }
}
