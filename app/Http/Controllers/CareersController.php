...
class CareersController extends Controller
{
    public function index()
    {
        try {
            ...
            $homePage = Page::where('slug', 'careers')->firstOrFail();
            ...
            Log::error('Database error in CareersController: ' . $e->getMessage());
            $sections = collect();
            $dbError = 'Koneksi ke database gagal.';
        } 
        catch (\Exception $e) {
            Log::error('Error umum di CareersController: ' . $e->getMessage());
            $sections = collect();
            $dbError = 'Terjadi kesalahan tak terduga.';
        }

        // 4. Kirim variabel ke view
        return view('pages.careers', compact('sections', 'dbError'));
    }
}
