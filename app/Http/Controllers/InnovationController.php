...
class innovationController extends Controller
{
    public function index()
    {
        try {
            ...
            $homePage = Page::where('slug', 'innovation')->firstOrFail();
            ...
            Log::error('Database error in innovationController: ' . $e->getMessage());
            $sections = collect();
            $dbError = 'Koneksi ke database gagal.';
        } 
        catch (\Exception $e) {
            Log::error('Error umum di innovationController: ' . $e->getMessage());
            $sections = collect();
            $dbError = 'Terjadi kesalahan tak terduga.';
        }

        // 4. Kirim variabel ke view
        return view('pages.innovation', compact('sections', 'dbError'));
    }
}
