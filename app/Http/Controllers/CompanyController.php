...
class CompanyController extends Controller
{
    public function index()
    {
        try {
            ...
            $homePage = Page::where('slug', 'company')->firstOrFail();
            ...
            Log::error('Database error in CompanyController: ' . $e->getMessage());
            $sections = collect();
            $dbError = 'Koneksi ke database gagal.';
        } 
        catch (\Exception $e) {
            Log::error('Error umum di CompanyController: ' . $e->getMessage());
            $sections = collect();
            $dbError = 'Terjadi kesalahan tak terduga.';
        }

        // 4. Kirim variabel ke view
        return view('pages.company', compact('sections', 'dbError'));
    }
}
